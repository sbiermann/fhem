...
#  $hash->{AttrFn}  = "HEATRONIC_Attr";
  ...
  $hash->{NotifyFn} = "HEATRONIC_Notify";
  $hash->{AttrList} =
...
HEATRONIC_Notify($$)
{
  ...
  return if( !$events );
  if( grep(m/^DISCONNECTED/, @{$events}) ) {
      Log3 ($hash, 3, "$name disconnect detected... trying to reconnect");
      DevIo_OpenDev($hash,1,"HEATRONIC_DoInit");
      return undef;
  }
  return undef;
}
